...
Class Decimoterceiro extends BaseController {
    ...
    public function index()
    {
        ...
        $dados['_titulo'] = " Antecipação 1 parcela do 13º Salário";
    ...
    public function novo(){
        ...
        $this->_breadcrumb->add('Nova Antecipação 1 parcela do 13º Salário');

        return parent::ViewPortal('variaveis/decimoterceiro/novo', $dados);
    }

    public function editar($id){
        $id = base64_decode($id);
     
        $dados['_titulo'] = "Editar Antecipação do 13º Salário";
        $dados['rh'] = parent::VerificaPerfil('GLOBAL_RH', false);
        ...
        $this->_breadcrumb->add('Editar Antecipação do 13º Salário');

        return parent::ViewPortal('variaveis/decimoterceiro/update', $dados);
    }
    ...
    public function save()
    {
        ...
        $request    = $this->request->getPost();
        $files = $this->request->getFiles(); // Pega todos os arquivos
        $valida     = $this->mParam->valida13($request);
        if($valida){
            exit(responseJson('error', 'O colaborador já possui uma solicitação de 13º esse ano.'));
        }
