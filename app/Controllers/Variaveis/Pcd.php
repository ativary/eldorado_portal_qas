...
Class PCD extends BaseController {
    ...
    public function index()
    {
        ...
        parent::VerificaPerfil('VARIAVEIS_PCD');
        $dados['_titulo'] = "Auxilio Excepcional";
        ...
        $dados['listaReqs']     = $this->mParam->getReq(4, $dados['data_inicio'],$dados['data_fim'],$dados['funcionario'], false,  $dados['situacao'] );
        $dados['listaSecao']    = $this->mParam->listaSecaoGestor(true);
        
        
        return parent::ViewPortal('variaveis/pcd/index', $dados);
    ...
    public function novo(){
        ...
        parent::VerificaPerfil('VARIAVEIS_PCD');
        $dados['_titulo'] = "Novo Auxilio Excepcional";
        $dados['rh'] = true;
        $dados['resFuncionarioSecao'] = $this->mParam->ListarFuncionariosSecao('all', $dados);
        $dados['param4']   = json_encode($this->mParam->getParametros(4));
        $dados['chapaFunc'] = util_chapa(session()->get('func_chapa'))['CHAPA'] ?? null;
        $this->_breadcrumb->add('Novo Auxilio Excepcional');

        return parent::ViewPortal('variaveis/pcd/novo', $dados);
    }

    public function editar($id){
       
        $id = base64_decode($id);
        $dados['_titulo'] = "Editar Auxilio Excepcional";
        $dados['rh'] = true;
        ...
        $dados['dependentes']     = $this->mParam->dadosFuncDepend($dados['req'][0]->chapa);
     
        $dados['valores']     = json_decode($dados['req'][0]->valores) ;
        $dados['param4']   = json_encode($this->mParam->getParametros(4));
        $dados['chapaFunc'] = util_chapa(session()->get('func_chapa'))['CHAPA'] ?? null;
        $this->_breadcrumb->add('Editar Auxilio Excepcional');

        return parent::ViewPortal('variaveis/pcd/update', $dados);
    }
    ...
    public function Anexos()
    {
        try {
            $request = $this->request->getPost();
            $Anexos = $this->mParam->historico($request['id']);
    ...
    public function dadosFunc()
    {
        ...
        $result2     = $this->mParam->dadosFuncDepend($request['chapa']);

         // Combine ambos os resultados em um array associativo
        $response = [
            'result1' => $result,
            'result2' => $result2
        ];

        // Codifique o array em JSON e envie-o de volta
