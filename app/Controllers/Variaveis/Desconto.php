...
Class Desconto extends BaseController {
    ...
    public function index()
    {
        ...
        parent::VerificaPerfil('VARIAVEIS_DESCONTO');
        $dados['_titulo'] = "Desconto Autorizado";
        ...
        $dados['listaReqs']     = $this->mParam->getReq(5, $dados['data_inicio'],$dados['data_fim'],$dados['funcionario'],false, $dados['situacao'] );
       
        $dados['listaSecao']    = $this->mParam->listaSecaoGestor( $dados['rh']);
       
        
        return parent::ViewPortal('variaveis/desconto/index', $dados);

    }

    public function novo(){
        parent::VerificaPerfil('VARIAVEIS_DESCONTO');
        $dados['_titulo'] = "Nova Desconto Autorizado";
        ...
        $dados['param6']   = json_encode($this->mParam->getParametros(5));
        $this->_breadcrumb->add('Nova Desconto Autorizado');

        return parent::ViewPortal('variaveis/desconto/novo', $dados);
    }
    public function substituicaoTermo()
    {
        ...
        $html = view('variaveis/desconto/termo',  $dados);
        $mpdf = new Mpdf([
            'mode' => 'utf-8',
            'format' => 'A4',
            'orientation' => 'P'
        ...
        $pdfContent = $mpdf->Output('', 'S'); // 'S' faz o Output para string (não download ou inline)
         ...
         ->setHeader('Content-Disposition', 'attachment; filename="autorizacao_Desconto.pdf"')
         ->setBody($pdfContent)
    ...
    public function editar($id){
        ...
        $dados['_titulo'] = "Editar Desconto Autorizado";
        $dados['rh'] = parent::VerificaPerfil('GLOBAL_RH', false);
        ...
        $this->_breadcrumb->add('Editar Desconto Autorizado');

        return parent::ViewPortal('variaveis/desconto/update', $dados);
    }
    ...
    public function salvarAnexo2()
    {
        try {
            ...
            $result3 = $this->mParam->VoltaRH($request['id']);
            // Converte o array em JSON
            exit(responseJson('success', 'Anexos salvos com sucesso.'));
