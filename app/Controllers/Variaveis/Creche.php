...
Class Creche extends BaseController {
    ...
    public function index()
    {
        ...
        parent::VerificaPerfil('VARIAVEIS_CRECHE');
        $dados['_titulo'] = "Auxilio Creche";
        ...
        if(!$dados['rh']){
            ...
            $dados['listaSecao']    = $this->mParam->listaSecaoGestor($dados['rh'],$gestor[0]['CHAPA'] );
        }else{
        ...
        $dados['listaReqs']     = $this->mParam->getReq(2, $dados['data_inicio'],$dados['data_fim'],$dados['funcionario'], false,  $dados['situacao'] );
       
        
        
        return parent::ViewPortal('variaveis/creche/index', $dados);
    ...
    public function novo(){
        ...
        parent::VerificaPerfil('VARIAVEIS_CRECHE');
        $dados['_titulo'] = "Novo Auxilio Creche";
        $dados['rh'] = parent::VerificaPerfil('GLOBAL_RH', false);
        if(!$dados['rh']){
            $gestor  = $this->mParam->funcGestor(util_chapa(session()->get('func_chapa'))['CHAPA'] ?? null);
        ...
        return parent::ViewPortal('variaveis/creche/novo', $dados);
    }

    public function editar($id){
       
        $id = base64_decode($id);
        $dados['_titulo'] = "Editar Auxilio Creche";
        $dados['rh'] = parent::VerificaPerfil('GLOBAL_RH', false);
        ...
        $dados['dependentes']     = $this->mParam->dadosFuncDepend($dados['req'][0]->chapa,2);
        if(!$dados['rh']){
            $gestor  = $this->mParam->funcGestor(util_chapa(session()->get('func_chapa'))['CHAPA'] ?? null);
        ...
        $dados['chapaFunc'] = util_chapa(session()->get('func_chapa'))['CHAPA'] ?? null;
        $dados['log_nome'] = session()->get('log_nome');
        $this->_breadcrumb->add('Editar Auxilio Creche');

        return parent::ViewPortal('variaveis/creche/update', $dados);
    }
    ...
    public function dadosFunc()
    {
        ...
        $result2     = $this->mParam->dadosFuncDepend($request['chapa'], 2,$result[0]['CODFILIAL'] );
