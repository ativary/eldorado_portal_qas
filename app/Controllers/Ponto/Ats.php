...
Class Ats extends BaseController {
    ...
    public function __construct()
    {
        ...
        $this->folderJSON   = $_SERVER['DOCUMENT_ROOT'].'\\writable\\cache\\';
	...
	public function cargaAtsMacro($jsonFile)
    {
        ...
        if(file_exists($this->folderJSON.$jsonFile)){
            ...
            $dados = json_decode(file_get_contents($this->folderJSON.$jsonFile));
            if($dados){
                //print_r($dados);
                $this->mPonto->cargaAtsMacro($dados);
                echo 'Carga do arquivo '.$jsonFile.' finalizada.<br>';
            } else {
                echo 'Arquivo '.$jsonFile.' inválido.<br>';
            }
        } else {
            echo 'Arquivo '.$this->folderJSON.$jsonFile.' não encontrado.<br>';
        }
    ...
    public function cargaAtsTotalizador($jsonFile)
    {
        ...
        if(file_exists($this->folderJSON.$jsonFile)){
            ...
            if($dados){
                //print_r($dados);
                $this->mPonto->cargaAtsTotalizador($dados);
                echo 'Carga do arquivo '.$jsonFile.' finalizada.<br>';
            } else {
                echo 'Arquivo '.$jsonFile.' inválido.<br>';
            }
