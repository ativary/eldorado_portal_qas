...
/**/										AND ISNULL((CASE WHEN CAL.HORAINICIO =0 AND CAL.HORAFINAL >= 1439 THEN P.extra_feriado ELSE extra_feriado_parcial END) ,0) = 0 THEN HTRAB 
/**/										
...
/**/									ELSE 0 END) 
- (CASE WHEN B.FERIADO = 0 THEN ISNULL(Z.HEXTRA_DIARIA,0) ELSE ISNULL((CASE WHEN CAL.HORAINICIO =0 AND CAL.HORAFINAL >= 1439 THEN 0 ELSE extra_feriado_parcial END) ,0) END) 
VALOR
...
/**/																						(SELECT MAX(DTMUDANCA) FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA <= B.DATA))
					...
					/***************************************************************************************************************/
						...
						FROM(
							...
							WHERE
									...
									--AND Z.CODINDICE = (CASE WHEN ZZ.INDICE + ((SELECT MAX(INDICE) FROM ABATHOR WHERE CODCOLIGADA = A.CODCOLIGADA AND CODHORARIO = AH.CODIGO)-1) > (SELECT MAX(INDICE) FROM ABATHOR WHERE CODCOLIGADA = A.CODCOLIGADA AND CODHORARIO = AH.CODIGO) THEN (ZZ.INDICE + ((SELECT MAX(INDICE) FROM ABATHOR WHERE CODCOLIGADA = A.CODCOLIGADA AND CODHORARIO = AH.CODIGO)-1)) - (SELECT MAX(INDICE) FROM ABATHOR WHERE CODCOLIGADA = A.CODCOLIGADA AND CODHORARIO = AH.CODIGO) ELSE (ZZ.INDICE + ((SELECT MAX(INDICE) FROM ABATHOR WHERE CODCOLIGADA = A.CODCOLIGADA AND CODHORARIO = AH.CODIGO)-1)) END)
									AND Z.CODINDICE = (CASE WHEN INDICE + (SELECT N.INDINICIOHOR FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA = (SELECT MAX(DTMUDANCA) FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA <= B.DATA)) > (SELECT MAX(INDICE) FROM ABATHOR WHERE CODCOLIGADA = A.CODCOLIGADA AND CODHORARIO = AH.CODIGO) THEN (INDICE + (SELECT N.INDINICIOHOR FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA = (SELECT MAX(DTMUDANCA) FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA <= B.DATA))) - (SELECT MAX(INDICE) FROM ABATHOR WHERE CODCOLIGADA = A.CODCOLIGADA AND CODHORARIO = AH.CODIGO) ELSE (INDICE + (SELECT N.INDINICIOHOR FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA = (SELECT MAX(DTMUDANCA) FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA <= B.DATA))) END) -1
								--AND B.HTRAB > 600
					...
					FROM(   
						SELECT
							...
							B.CODSECAO,
							D.DESCRICAO NOMESECAO,
							ISNULL((SELECT DATEDIFF(MINUTE, INICIO, FIM) BH FROM AOCORRENCIACALCULADA OBH WHERE OBH.TIPOOCORRENCIA = 'ECA' AND OBH.CODCOLIGADA = B.CODCOLIGADA AND OBH.CHAPA = B.CHAPA AND OBH.DATAREFERENCIA = A.DATA AND OBH.ATITUDE = 1),0) TOTALBH
...
SELECT 
	...
	CONCAT('(IntraJornada) previsto ',dbo.mintotime(PREVISTO),', realizado ',
...
WHERE 
     ISNULL((CASE WHEN (CASE WHEN isnull((CASE WHEN TRES = 0 THEN 1440 ELSE TRES END),'')> 0 THEN (CASE WHEN TRES = 0 THEN 1440 ELSE TRES END) - DOIS ELSE NULL END)< 0  
	        THEN (CASE WHEN TRES = 0 THEN 1440 ELSE TRES END) + (1440 - DOIS)
	        ELSE (CASE WHEN isnull((CASE WHEN TRES = 0 THEN 1440 ELSE TRES END),'')> 0 THEN (CASE WHEN TRES = 0 THEN 1440 ELSE TRES END) - DOIS ELSE NULL END) END), 0) < PREVISTO

      AND QTD >=4 AND PREVISTO >= 60
						...
						****/
						...
						/***************************************************************************************************************/ 
						  ...
						  SELECT
							  ...
							  , '5 dias de ' + CONVERT(VARCHAR(10),DATA,103) + ' à ' + CONVERT(VARCHAR(10),DATAFINAL,103)   COMPLEMENTO
...
SELECT 
	...
	HOR_HIST,
    'interjornada' OCORRENCIA,
    CONCAT('Tempo Mínimo de refeição (IntraJornada) ',dbo.mintotime(PREVISTO),', realizado ',
    ISNULL(dbo.mintotime((CASE WHEN (CASE WHEN isnull(TRES,'')> 0 THEN TRES - DOIS ELSE NULL END)< 0 
	        THEN TRES + (1440 - DOIS)
	        ELSE (CASE WHEN isnull(TRES,'')> 0 THEN TRES - DOIS ELSE NULL END) END)), '00:00'),', suprimido ', dbo.mintotime((PREVISTO - (CASE WHEN (CASE WHEN isnull(TRES,'')> 0 THEN TRES - DOIS ELSE NULL END)< 0 
	        THEN TRES + (1440 - DOIS)
	        ELSE (CASE WHEN isnull(TRES,'')> 0 THEN TRES - DOIS ELSE NULL END) END))))COMPLEMENTO
    , '' VALOR
	, NULL BASE
	, NULL HTRAB
	, NULL EXTRAAUTORIZADO
...
// exit();
	...
	public function Workflow_original(){
	...
	public function Workflow2_original(){
...
X.COMPLEMENTO
		...
		/***************************************************************************************************************/
			...
			FROM (
			 ...
			 WHERE XX.HORARIO COLLATE Latin1_General_CI_AS NOT IN(SELECT CODIGO FROM ".DBPORTAL_BANCO."..zcrmportal_ocorrencia_horario WHERE coligada =  (SELECT COLIGADA FROM PAR)  )
		...
		/***************************************************************************************************************/
			...
			FROM (
			 ...
			 WHERE XX.HORARIO COLLATE Latin1_General_CI_AS  IN(SELECT CODIGO FROM ".DBPORTAL_BANCO."..zcrmportal_ocorrencia_horario WHERE coligada =  (SELECT COLIGADA FROM PAR)  )
    ...
    public function ListarOcorrenciaArquivoMorto($dados){
		...
		*/
         ...
         $query = "
					...
					".DBPORTAL_BANCO."..zcrmportal_ocorrencia (NOLOCK) Z 
...
/*************************************************************************************
					...
					)X ON Z.chapa = X.CHAPA COLLATE Latin1_General_CI_AS AND Z.codcoligada = X.CODCOLIGADA AND Z.data = X.DATA AND Z.ocorrencia = X.OCORRENCIA
					
					LEFT JOIN PFUNC B ON Z.chapa = B.CHAPA COLLATE Latin1_General_CI_AS AND Z.codcoligada = B.CODCOLIGADA
					...
					WHERE 
					  		Z.codmotivo IS NOT NULL  
					  	AND X.chapa IS NULL
						AND Z.data BETWEEN '{$data_inicio}' AND '{$data_fim}'
...
// exit();
	...
	public function ConfiguracaoHorario($request)
	{
		...
		if($request['field'] != 'escala_especial' && $request['field'] != 'tipo_horario') $request['marcado'] = h2m($request['marcado']);
