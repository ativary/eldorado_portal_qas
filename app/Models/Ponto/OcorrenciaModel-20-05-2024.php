...
class OcorrenciaModel extends Model {
    ...
    public function ListarPeriodoPonto(){
		...
		try{
			...
			$query = " SELECT dtini, dtfim FROM zcrmportal_ponto_config WHERE idemp = '{$this->coligada}' ";
			$result = $this->dbportal->query($query);
			if(($result->getNumRows() <= 0)) return false;
			$ConfigPonto = $result->getResultArray()[0];

			$periodo_ini = date('Y-m-d', strtotime($ConfigPonto['dtini']));
...
FROM( 
 ...
 /*02/05/2024*/
	    ,(
	       (CASE
	          WHEN HEXTRA_DIARIA <= 15 AND ENT1 < (ENTRADA1 - TOLERANCIA_INICIO) THEN 'Fora da tolerância - '  + dbo.MINTOTIME(ENTRADA1 - ENT1)
	          WHEN HEXTRA_DIARIA <= 15 AND SAI > (SAIDA2 + TOLERANCIA_FIM)      THEN  'Fora da tolerância - ' + dbo.MINTOTIME(SAI - SAIDA2 )
	    ...
	    ,'Fora da tolerância - '  + dbo.MINTOTIME((((CASE
	          ...
	          ELSE 0 END) + 
            (CASE
	          WHEN HEXTRA_DIARIA <= 15 AND (CASE WHEN SAI < ENTRADA1 THEN SAI+1440 ELSE SAI END)  > (SAIDA2 + TOLERANCIA_FIM) THEN  ((CASE WHEN SAI < ENTRADA1 THEN SAI+1440 ELSE SAI END) - SAIDA2 )
    ...
    WHERE 
		...
		".str_replace('A.CODFILIAL', 'B.CODFILIAL', $where_filial)."
		 --AND A.CHAPA = '050002721'
   		...
   		LEFT JOIN Z_OUTSERV_MELHORIAS3 E ON E.CODCOLIGADA = TBL.COL AND E.CODHORARIO = TBL.HOR AND E.CODINDICE =  TBL.IND_HOR
  )XX
...
FROM( 
	SELECT
	    ...
	    ,(
	       (CASE
	          WHEN HEXTRA_DIARIA <= 15 AND ENT1 < (ENTRADA1 - TOLERANCIA_INICIO) THEN 'DIARIO ANTES - '  + dbo.MINTOTIME(ENTRADA1 - ENT1)
	          WHEN HEXTRA_DIARIA <= 15 AND SAI > (SAIDA2 + TOLERANCIA_FIM)      THEN  'DIARIO DEPOIS - ' + dbo.MINTOTIME(SAI - SAIDA2 )
	          WHEN HEXTRA_DIARIA <= 15 AND SAI IS NULL THEN 'SEM SAIDA'
	     ELSE 'N/A' END))COMPLEMENTO
...
SELECT 
	  ...
	  , DESCANSO
	  , FERIADO
      , ISNULL(MT.limite_jornada,0) HEXTRA_DIARIA  
      , QTD2  
...
WHERE BAT % 2 = 0 /*SOMENTE BATIDAS PAR???*/
    ...
    WHERE  
	   ISNULL(HTRAB,0) > ISNULL(HEXTRA_DIARIA,0)
       AND (SELECT excesso_jornada FROM ".DBPORTAL_BANCO."..zcrmportal_ocorrencia_tipo where id = 1) = 1
	   -----AND isnull(BASE,0) BETWEEN 1 AND 540
	   AND HEXTRA_DIARIA <= 15
	   AND COMPLEMENTO <> 'N/A'
					...
					FROM(   
						SELECT
							...
							dbo.CalculoIndiceHorarioV2(
								DATEDIFF(DAY, AH.DATABASEHOR, A.DATA),
								(SELECT N.INDINICIOHOR FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA = (SELECT MAX(DTMUDANCA) FROM PFHSTHOR (NOLOCK) N WHERE A.CODCOLIGADA = N.CODCOLIGADA AND A.CHAPA = N.CHAPA AND N.DTMUDANCA <= A.DATA)),
								(SELECT MAX(CODINDICE) FROM Z_OUTSERV_MELHORIAS3 WHERE CODHORARIO = AH.CODIGO AND CODCOLIGADA = A.CODCOLIGADA GROUP BY CODHORARIO, CODCOLIGADA)
							)
							IND
...
/**/								WHERE (XX.VALOR > ISNULL(N.extra_feriado,0) or XX.VALOR > ISNULL(N.extra_feriado_parcial,0)) AND XX.VALOR > 0
					...
					/***************************************************************************************************************/
						...
						FROM (
							...
							WHERE	
								(AX.FERIAS IS NOT NULL OR AX.AFAST IS NOT NULL) AND AX.BAT > 0 /*AND ( ABONO = 0 OR ABONO >= 240) removido a pedido da marina 15/05/2024 21:35, solicitação do Deivison via teams*/ /* A pedido ignorado se o abono foi menor que 4horas*/
					...
					FROM(
						...
						/*
						  ...
						  */
						  ...
						  /*30/04/2024*/  /*4 BAT FRACIONADO*/
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 4 AND PLAN_INIC_HOR = PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 4 AND SAI1 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ENT2 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 4 AND SAI1 < PLAN_INIC_HOR AND ENT2-SAI1 < INTERVALO_HOR  THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))  + ' antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 4 AND ENT2 > PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR   THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))  + ' depois do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 4 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND SAI1 < PLAN_INIC_HOR AND ENT2-SAI1 = INTERVALO_HOR THEN 'Intervalo correto antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 4 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND ENT2 > PLAN_FIM_HOR AND ENT2-SAI1 = INTERVALO_HOR  THEN 'Intervalo correto depois do planejado'
								 
								
								   /*6 BAT FRACIONADO*/
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 6 AND PLAN_INIC_HOR = PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))
							       WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 6 AND SAI1 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ENT3 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)) < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - ((ENT2-SAI1)+(ENT3-SAI2)))
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 6 AND SAI1 < PLAN_INIC_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)) < INTERVALO_HOR  THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - ((ENT2-SAI1)+(ENT3-SAI2)))  + ' antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 6 AND ENT3 > PLAN_FIM_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)) < INTERVALO_HOR   THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - ((ENT2-SAI1)+(ENT3-SAI2)))  + ' depois do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 6 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND SAI1 < PLAN_INIC_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)) > INTERVALO_HOR THEN 'Intervalo correto antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 6 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND ENT3 > PLAN_FIM_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)) = INTERVALO_HOR  THEN 'Intervalo correto depois do planejado'
								 
									 
								   /*8 BAT FRACIONADO*/
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 8 AND PLAN_INIC_HOR = PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 8 AND SAI1 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ENT4 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)) < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)))
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 8 AND SAI1 < PLAN_INIC_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)) < INTERVALO_HOR  THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)))  + ' antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 8 AND ENT4 > PLAN_FIM_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)) < INTERVALO_HOR   THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)))  + ' depois do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 8 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND SAI1 < PLAN_INIC_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)) > INTERVALO_HOR THEN 'Intervalo correto antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FRACIONADO' AND BAT = 8 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND ENT4 > PLAN_FIM_HOR AND ((ENT2-SAI1)+(ENT3-SAI2)+(ENT4-SAI3)) = INTERVALO_HOR  THEN 'Intervalo correto depois do planejado'  
								 
								   /*4 BAT FIXO*/
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FIXO' AND BAT = 4 AND PLAN_INIC_HOR = PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FIXO' AND BAT = 4 AND SAI1 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ENT2 BETWEEN PLAN_INIC_HOR AND PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR THEN 'Suprimido ' + dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FIXO' AND BAT = 4 AND SAI1 < PLAN_INIC_HOR AND ENT2-SAI1 < INTERVALO_HOR  THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))  + ' antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FIXO' AND BAT = 4 AND ENT2 > PLAN_FIM_HOR AND ENT2-SAI1 < INTERVALO_HOR   THEN 'Suprimido '+ dbo.MINTOTIME(INTERVALO_HOR - (ENT2-SAI1))  + ' depois do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FIXO' AND BAT = 4 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND SAI1 < PLAN_INIC_HOR AND ENT2-SAI1 = INTERVALO_HOR THEN 'Intervalo correto antes do planejado'
								   WHEN /*MOTORISTA = 'NAO' AND*/ FRACIONADO = 'FIXO' AND BAT = 4 AND PLAN_INIC_HOR <> PLAN_FIM_HOR AND ENT2 > PLAN_FIM_HOR AND ENT2-SAI1 = INTERVALO_HOR  THEN 'Intervalo correto depois do planejado'
					 ...
					 /*30/04/2024*/	 /*MOTORISTA*/     
							...
							WHERE (SELECT ISNULL(P.intervalo_obrigatorio,0) FROM ".DBPORTAL_BANCO."..zcrmportal_ocorrencia_horario P WHERE coligada = COL AND codigo = HOR COLLATE Latin1_General_CI_AS) = 1
			...
			-----------------
				WITH MEUHORARIO AS (
					...
					SELECT
						...
						CODINDICE
					FROM
					dbo.CALCULO_HORARIO(CONVERT(VARCHAR,@DATABASEHOR,103), @CODIGO, 1, 1, DATEDIFF(DAY, @DATABASEHOR, '2050-12-31'))
			)
