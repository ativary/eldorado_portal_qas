    ...
    public function saveAtsMacro($dados)
    {
        ...
        if($dados){
            foreach($dados as $key => $Macro){
                ...
                /*echo '<pre>qtde:' . $existe.'<br>';
                echo 'cpf: '.$Macro->cpf.'<br>';
                echo 'data_inicio_status: '.str_replace(['T', 'Z'], [' ', ''], $Macro->data_inicio_status).'<br>';
                echo 'status: '.$Macro->status.'<br>';*/

                if($existe <= 0 && strlen(trim($Macro->cpf)) > 0){

                    $placa = (strlen(trim($Macro->placa)) > 7) ? '' : $Macro->placa;

                    $insert = $this->dbportal
                        ->table('zcrmportal_ponto_ats_macro')
                        ->insert([
                            'cpf' => $Macro->cpf,
                            'status' => $Macro->status,
