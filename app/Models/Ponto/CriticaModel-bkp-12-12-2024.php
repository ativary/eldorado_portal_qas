...
class CriticaModel extends Model {
   ...
   //---------------------------------- critica
    ...
    ISNULL(FALTA_CASE,0) FALTA_CASE
		...
		, EXTRAEXECUTADO_CASE, SEM_PAR_CORRESPONDENTE, INTERJORNADA, JORNADA_MAIOR_10HORAS, SEM_PAR_CORRESPONDENTE_DESC, INTERJORNADA_DESC, JORNADA_MAIOR_10HORAS_DESC, BANCO_HORA,STATUS, JUSTIFICATIVA_BATIDA
			FROM (
  ...
  public function listaDataCritica($dados = false, $codloja = false, $excel = false) {
    ...
    try{
      ...
      FALTA_CASE*/
      ISNULL(FALTA_CASE,0) FALTA_CASE
      , EXTRAEXECUTADO_CASE, SEM_PAR_CORRESPONDENTE, INTERJORNADA, JORNADA_MAIOR_10HORAS, SEM_PAR_CORRESPONDENTE_DESC, INTERJORNADA_DESC, JORNADA_MAIOR_10HORAS_DESC,BANCO_HORA, JUSTIFICATIVA, JUSTIFICATIVA_CODIGO, QTDE_ABONO, ABONO_PENDENTE_RH, JUSTIFICATIVA_ATITUDE, USABANCOHORAS, CPF, CODSITUACAO, IS_MOTORISTA
        FROM (
          ...
          SELECT
            ...
            ,(CASE WHEN (B.DATADEMISSAO IS NULL OR B.DATADEMISSAO >= '{$dtfim}') THEN 'Ativo' ELSE 'Demitido' END) CODSITUACAO
