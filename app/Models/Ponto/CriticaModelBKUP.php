...
class CriticaModel extends Model {
   ...
   //---------------------------------- critica
    ...
    $FT_BANCOHORA = " AND X.BANCO_HORA IS NULL ";
    if($bancohoras){
    ...
    CHAPA, NOME, DATA, DATAREFERENCIA, DATAREFERENCIA2, IDAAFDT, BATIDA, NATUREZA,
		...
		, EXTRAEXECUTADO_CASE, SEM_PAR_CORRESPONDENTE, INTERJORNADA, JORNADA_MAIOR_10HORAS, SEM_PAR_CORRESPONDENTE_DESC, INTERJORNADA_DESC, JORNADA_MAIOR_10HORAS_DESC, BANCO_HORA,STATUS
			FROM (
          ...
          {$filtro_secao}
				...
				WHERE
					(ATRASO_CASE IS NOT NULL OR FALTA_CASE IS NOT NULL OR EXTRAEXECUTADO_CASE  IS NOT NULL OR SEM_PAR_CORRESPONDENTE  IS NOT NULL OR INTERJORNADA  IS NOT NULL OR JORNADA_MAIOR_10HORAS  IS NOT NULL)
				{$FT_GLOBAL}
				{$FT_BANCOHORA}
				ORDER BY DATA, BATIDA ASC
		";

    // if($_SESSION['log_id'] == 2767) echo '<pre>'.$query.'</pre>';exit();
  ...
  public function listaDataCritica($dados = false, $codloja = false, $excel = false) {
    ...
    try{
      ...
      $query = "
      SELECT DISTINCT {$filtro_top1000} CHAPA, NOME, CODTIPO, CODFILIAL, DATA, BATIDAS_PORTAL, {$select_escala}
      ...
      (CASE WHEN FALTA_CASE = 0 THEN 0
        ...
        WHEN FALTA_JUST > 0 AND FALTA_JUST > FALTA_CASE THEN FALTA_CASE ELSE FALTA_CASE END )
      FALTA_CASE
      , EXTRAEXECUTADO_CASE, SEM_PAR_CORRESPONDENTE, INTERJORNADA, JORNADA_MAIOR_10HORAS, SEM_PAR_CORRESPONDENTE_DESC, INTERJORNADA_DESC, JORNADA_MAIOR_10HORAS_DESC,BANCO_HORA, JUSTIFICATIVA, JUSTIFICATIVA_CODIGO, QTDE_ABONO, ABONO_PENDENTE_RH, JUSTIFICATIVA_ATITUDE
        FROM (
          ...
          SELECT
            ...
            (CASE WHEN FALTA > '0' THEN FALTA ELSE NULL END) FALTA_CASE,
            ISNULL((SELECT SUM(FIM-INICIO) FROM AJUSTFUN D (NOLOCK) WHERE A.CODCOLIGADA = D.CODCOLIGADA AND A.CHAPA = D.CHAPA AND A.DATA = D.DATA AND TIPOOCORRENCIA IN ('F') AND JUSTIFICATIVA IS NULL),0) FALTA_JUST,
    ...
    public function listaBatidasApontadaCritica($dados) {
      ...
      $query = " SELECT id, chapa, dtponto, ent1, ent2, ent3, ent4, ent5, sai1, sai2, sai3, sai4, sai5, status, motivo_reprova, COALESCE(natent1, natent2, natent3, natent4, natent5, natsai1, natsai2, natsai3, natsai4, natsai5) natureza, COALESCE(dtrefent1, dtrefent2, dtrefent3, dtrefent4, dtrefent5, dtrefsai1, dtrefsai2, dtrefsai3, dtrefsai4, dtrefsai5) data_referencia FROM zcrmportal_ponto_horas (NOLOCK) WHERE coligada = '{$this->coligada}' AND status IN ('1','2','3','R') 
        and dtponto BETWEEN '{$dtini}' AND '{$dtfim}' AND dt_delete IS NULL
        AND EXISTS (SELECT B.CHAPA FROM ".DBRM_BANCO."..PFUNC B (NOLOCK) WHERE B.CODSITUACAO NOT IN ('D') AND CODCOLIGADA = '{$this->coligada}' {$qr_secao})
       ";
    ...
    public function ListarCriticaPeriodoRM(){
    ...
    public function listaAbonoCritica($dataInicio, $dataTermino, $tipo)
    {
      ...
      if($dataInicio === NULL || $dataTermino === NULL) return false;
        ...
        $query = " SELECT id, dtponto, abn_horaini, abn_horafim, abn_codabono, status, justificativa_abono_tipo, motivo_reprova, chapa, atitude_justificativa FROM zcrmportal_ponto_horas WHERE coligada = '{$coligada}' AND movimento IN ({$ft_tipo}) AND dtponto BETWEEN '{$dataInicio}' AND '{$dataTermino}' AND status NOT IN ('S') AND usu_delete IS NULL ";
        $result = $this->dbportal->query($query);
        if(!$result) return false;
        return ($result->getNumRows() > 0) 
                ? $result->getResultArray() 
    ...
    public function alteraAtitude($request)
    {
        ...
        if($movimento == 8 || $movimento == 7){
            ...
            $query = $this->dbportal->query(" INSERT INTO zcrmportal_ponto_horas 
                  (dtponto,atitude_dt,atitude_ini,atitude_fim, dtcadastro, chapa, coligada, status, movimento, usucad, atitude_justificativa, atitude_tipo) VALUES 
                  ('". $request['data']  ."','".$request['data'] ."','0','".h2m($request['horas'])."', '".date('Y-m-d H:i:s')."','".$request['chapa']."','".$this->coligada."', '1', '{$movimento}', '".$this->log_id."', '{$request['tipo']} - Altera Atitude', '{$request['atitude']}')");
        ...
        $query = $this->dbportal->query(" INSERT INTO zcrmportal_ponto_horas 
          (dtponto,abn_dtini,abn_dtfim,abn_horaini,abn_horafim,abn_codabono, dtcadastro, chapa, coligada, status, movimento, usucad, justificativa_excecao, codfilial, id_hierarquia_gestor, aprgestor_data, aprgestor_user, gestor_dtapr, gestor_usuapr) VALUES 
          ('". $request['data']  ."','". $request['data']  ."','". $request['data']  ."',0,'".h2m($request['horas'])."','{$codabono}', '".date('Y-m-d H:i:s')."','".$request['chapa']."','".$this->coligada."', '1', '{$movimento}', '".$this->log_id."', 'Falta Não Remunerada', '1', NULL, NULL, NULL, NULL, NULL)
          ");
