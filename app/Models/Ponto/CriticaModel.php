		...
		, EXTRAEXECUTADO_CASE, SEM_PAR_CORRESPONDENTE, INTERJORNADA, JORNADA_MAIOR_10HORAS, JORNADA_MAIOR_12HORAS, SEM_PAR_CORRESPONDENTE_DESC, INTERJORNADA_DESC, JORNADA_MAIOR_10HORAS_DESC, JORNADA_MAIOR_12HORAS_DESC, BANCO_HORA,STATUS, JUSTIFICATIVA_BATIDA, OBS
          ...
          ISNULL((SELECT COUNT(CHAPA) FROM AABONFUN WHERE CHAPA = A.CHAPA AND CODCOLIGADA = A.CODCOLIGADA AND DATA = A.DATA AND CODABONO = '".((DBRM_BANCO == 'CorporeRMPRD') ? '031' : '030')."'),0) FALTA_JUST,
            ...
            FROM (
              SELECT
                  AA.CODAVISO
                ,(SELECT MAX(CONVERT(DATETIME, CONVERT(CHAR, ISNULL(CASE WHEN DATA > DATAREFERENCIA THEN DATAREFERENCIA +1 ELSE DATAREFERENCIA END,DATA),23) + ' ' + CONVERT(CHAR, dbo.MINTOTIME(BATIDA),8))) FROM ABATFUN M WHERE AA.CODCOLIGADA = M.CODCOLIGADA AND AA.CHAPA = M.CHAPA AND AA.DATAREFERENCIA-1 = M.DATAREFERENCIA AND NATUREZA = 1) BAT_anterior
                ,(SELECT MIN(CONVERT(DATETIME, CONVERT(CHAR, ISNULL(CASE WHEN DATA > DATAREFERENCIA THEN DATAREFERENCIA +1 ELSE DATAREFERENCIA END,DATA),23) + ' ' + CONVERT(CHAR, dbo.MINTOTIME(BATIDA),8))) FROM ABATFUN M WHERE AA.CODCOLIGADA = M.CODCOLIGADA AND AA.CHAPA = M.CHAPA AND AA.DATAREFERENCIA = M.DATAREFERENCIA AND NATUREZA = 0) BAT_atual
              FROM 
                AAVISOCALCULADO AA (NOLOCK)
              WHERE 
                    AA.CODCOLIGADA = AA.CODCOLIGADA
                AND AA.CHAPA = A.CHAPA
                AND AA.DATAREFERENCIA = A.DATA
                AND AA.CODAVISO = '1' 
            )YY
              WHERE (CASE WHEN DBO.MINTOTIME(DATEDIFF(minute ,YY.BAT_anterior, YY.BAT_atual)) IS NULL or DATEDIFF(minute, YY.BAT_anterior, YY.BAT_atual) > 660 THEN 'Bat.Inválida'
              ELSE DBO.MINTOTIME(DATEDIFF(minute ,YY.BAT_anterior, YY.BAT_atual)) END) NOT IN ('Bat.Inválida')
