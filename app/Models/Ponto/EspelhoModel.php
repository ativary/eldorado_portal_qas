                ...
                if($Dados['tipo'] == "U"){
                    ...
                    $query = "
                        ...
                        SET 
                            ...
                            dtrefent4 = CASE WHEN dtrefent4 IS NOT NULL THEN '".dtEn($Dados['data_ref'], true)."' ELSE NULL END,
                            dtrefsai1 = CASE WHEN dtrefsai1 IS NOT NULL THEN '".dtEn($Dados['data_ref'], true)."' ELSE NULL END,
                            dtrefsai2 = CASE WHEN dtrefsai2 IS NOT NULL THEN '".dtEn($Dados['data_ref'], true)."' ELSE NULL END,
                            dtrefsai3 = CASE WHEN dtrefsai3 IS NOT NULL THEN '".dtEn($Dados['data_ref'], true)."' ELSE NULL END,
                            dtrefsai4 = CASE WHEN dtrefsai4 IS NOT NULL THEN '".dtEn($Dados['data_ref'], true)."' ELSE NULL END
                        WHERE 
                                ".h2m($Dados['batida_default'])." = COALESCE(ent1, ent2, ent3, ent4, ent5, sai1, sai2, sai3, sai4, sai5)
                            AND COALESCE(dtrefent1, dtrefent2, dtrefent3, dtrefent4, dtrefent5, dtrefsai1, dtrefsai2, dtrefsai3, dtrefsai4, dtrefsai5) = '".dtEn($Dados['data_ref_default'], true)."'
                            AND movimento = '1'
                            AND status not in ('S')
                            AND coligada = '{$coligada}'
                            AND chapa = '{$Dados['chapa']}'
                    ...
                    $query = "
                        ...
                        WHERE 
                            ...
                            AND movimento ='1'
                            AND status NOT IN ('S')
                            AND COALESCE(dtrefent1, dtrefent2, dtrefent3, dtrefent4, dtrefent5, dtrefsai1, dtrefsai2, dtrefsai3, dtrefsai4, dtrefsai5) = '".dtEn($Dados['data_ref'], true)."'
                    ";
                    $result = $this->dbportal->query($query);
                    if($result->getNumRows() > 0){
        ...
        if($dados){
            foreach($dados as $key => $Totalizador){
				...
				$existe = $this->dbportal
                    ...
                    ->where('data_gravacao', $Totalizador->data_gravacao)
                    ->get();
    ...
    public function CarregaMacro($request)
    {
        ...
        $result = $this->dbportal
            ...
            ->select("status, CONCAT(CONVERT(VARCHAR, data_inicio_status, 103),' ', CONVERT(VARCHAR, data_inicio_status, 8)) data_inicio_status, CONCAT(CONVERT(VARCHAR, data_fim_status, 103),' ', CONVERT(VARCHAR, data_fim_status, 8)) data_fim_status, tempo, placa, CONCAT(CONVERT(VARCHAR, data_gravacao, 103),' ', CONVERT(VARCHAR, data_gravacao, 8)) data_gravacao, 
                case 
                    when tipo_mensagem = 'Mensagem mobile' then 'Mobile'
                    when tipo_mensagem = 'Mensagem rastreador' then 'Mensagem'
                    when tipo_mensagem = 'Automatizacao rastreador' then 'Automatização'
                    else tipo_mensagem
                end tipo_mensagem", false)
            ->where("data_inicio_status BETWEEN '{$perIni}' AND '{$perFim}'")
            ->where('cpf', $request['cpf'])
            ->where('tipo_mensagem !=', 'Inserção Manual')
