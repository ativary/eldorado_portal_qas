...
FROM (
    SELECT P.CPF,
      ...
      ISNULL(
        (
          ...
          WHERE CHAPA = A.CHAPA
            ...
            AND DATA = A.DATA
            AND CODABONO = '".((DBRM_BANCO == ' CorporeRMPRD ') ? ' 031 ' : ' 030 ')."'
        ),
        0
      ) FALTA_JUST,
      (
      ...
      (
        ...
        FROM ".DBPORTAL_BANCO."..zcrmportal_ponto_justificativa_func AA (NOLOCK)
          INNER JOIN ".DBPORTAL_BANCO."..zcrmportal_ponto_motivos BB (NOLOCK) ON AA.justificativa = BB.id
      ...
      (
        ...
        FROM ".DBPORTAL_BANCO."..zcrmportal_ponto_horas AA
...
(
        SELECT COUNT(id)
        FROM ".DBPORTAL_BANCO."..zcrmportal_ocorrencia_motorista m
    ...
    FROM AAFHTFUN A (NOLOCK)
      LEFT JOIN ".DBPORTAL_BANCO."..zcrmportal_ponto_justificativa_func D (NOLOCK) ON A.DATA = D.dtponto
    ...
    WHERE A.CODCOLIGADA = B.CODCOLIGADA
      ...
      AND -- A.DATA BETWEEN {$periodo}
      A.DATA BETWEEN '{$dtini}' AND '{$dtfim}' { $FT_FUNCIONARIO } { $qr_secao } { $filtro_secao }
      /*AND B.CODSITUACAO <> 'D'*/
      AND (
      ...
      AND (
        ...
        WHERE CODCOLIGADA = A.CODCOLIGADA
          ...
          AND APROVADO = '1'
      ) <= 0 -- AND (B.DATADEMISSAO > (GETDATE() - 30) OR B.DATADEMISSAO IS NULL)
  ) X
WHERE (
