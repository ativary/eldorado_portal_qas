...
,CASE 
	...
	DBO.MINTOTIME(ISNULL(EXTRAEXECUTADO,0) - ISNULL(HEXTRA_DIARIA,0) - ISNULL(BH,0))
	ELSE COMPLEMENTO
END COMPLEMENTO

,CASE 
	WHEN COMPLEMENTO2 = 'FORA' THEN ISNULL(EXTRAEXECUTADO,0) - ISNULL(HEXTRA_DIARIA,0) - ISNULL(BH,0)
	ELSE VALOR
END VALOR
...
AND (ISNULL(EXTRAEXECUTADO,0) - ISNULL(HEXTRA_DIARIA,0) - ISNULL(BH,0)) > 0
...
SELECT
	...
	(
		SELECT
			(CASE WHEN (PF.DATADEMISSAO IS NULL OR PF.DATADEMISSAO >= '".$data_fim."') THEN 'Ativo' ELSE 'Demitido' END) CODSITUACAO
		FROM
			PFUNC PF
		WHERE
				PF.CHAPA = X.CHAPA
			AND PF.CODCOLIGADA = X.CODCOLIGADA
	) CODSITUACAO,
	(
		SELECT 
			MAX(G.NOME)
