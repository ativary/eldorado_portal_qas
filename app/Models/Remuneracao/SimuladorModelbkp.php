...
class SimuladorModel extends Model {
    ...
    public function ListarPosicaoRequisicaoAQ(){
        ...
        if($perfil_recrutamento) $where_perfil = "";
    ...
    public function ExecutaSimuladorCalculo($acao, $chapa = null, $posicao_destino = null, $adicional = array()){
        ...
        $ADICIONAL_NOTURNO = (strlen(trim($adicional['adicional_noturno'] ?? '') > 0 && $chapa == null) ? str_replace(',', '.', str_replace('.', '', $adicional['adicional_noturno'])) : "
            ISNULL((
                ...
                FROM 
                    PFFINANC PF (NOLOCK)
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL ZM1 ON ZM1.CODCOLIGADA = PF.CODCOLIGADA AND ZM1.CODIGO = '008'
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL_EV ZM2 ON ZM2.CODCOLIGADA = ZM1.CODCOLIGADA AND ZM2.CODIGO = ZM1.CODIGO
                    
                WHERE
                        CHAPA IN (SELECT CHAPA FROM PFUNC WHERE 1 = (CASE WHEN P.CHAPA = CHAPA THEN 1 WHEN P.CHAPA = '' AND CODSECAO = D2.codsecao COLLATE Latin1_General_CI_AS THEN 1 ELSE 0 END) AND CODCOLIGADA = P.CODCOLIGADA AND CODSITUACAO <> 'D')
                    ...
                    --AND PF.REF = CASE WHEN ZM1.TIPO = '03' THEN 30 ELSE PF.REF END
        ...
        $HE50 = (strlen(trim($adicional['he50_manual'] ?? '') > 0 && $chapa == null) ? str_replace(',', '.', str_replace('.', '', $adicional['he50_manual'])) : "
            ISNULL((
                ...
                FROM 
                    PFFINANC PF (NOLOCK)
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL ZM1 ON ZM1.CODCOLIGADA = PF.CODCOLIGADA AND ZM1.CODIGO = '002'
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL_EV ZM2 ON ZM2.CODCOLIGADA = ZM1.CODCOLIGADA AND ZM2.CODIGO = ZM1.CODIGO
        ...
        $HE80 = (strlen(trim($adicional['he80_manual'] ?? '') > 0 && $chapa == null) ? str_replace(',', '.', str_replace('.', '', $adicional['he80_manual'])) : "
                ...
                FROM 
                    PFFINANC PF (NOLOCK)
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL ZM1 ON ZM1.CODCOLIGADA = PF.CODCOLIGADA AND ZM1.CODIGO = '003'
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL_EV ZM2 ON ZM2.CODCOLIGADA = ZM1.CODCOLIGADA AND ZM2.CODIGO = ZM1.CODIGO
        ...
        $HE100 = (strlen(trim($adicional['he100_manual'] ?? '') > 0 && $chapa == null) ? str_replace(',', '.', str_replace('.', '', $adicional['he100_manual'])) : "
                ...
                FROM 
                    PFFINANC PF (NOLOCK)
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL ZM1 ON ZM1.CODCOLIGADA = PF.CODCOLIGADA AND ZM1.CODIGO = '004'
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL_EV ZM2 ON ZM2.CODCOLIGADA = ZM1.CODCOLIGADA AND ZM2.CODIGO = ZM1.CODIGO
        ...
        $MEDIA_DSR_HE = "
            ISNULL((
                ...
                FROM 
                    PFFINANC PF (NOLOCK)
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL ZM1 ON ZM1.CODCOLIGADA = PF.CODCOLIGADA AND ZM1.CODIGO = '007'
                    LEFT JOIN ZMDCONFSIMULACAOPORTAL_EV ZM2 ON ZM2.CODCOLIGADA = ZM1.CODCOLIGADA AND ZM2.CODIGO = ZM1.CODIGO
        ...
        SELECT
            ...
            FROM 
                PFFINANC PF (NOLOCK)
                LEFT JOIN ZMDCONFSIMULACAOPORTAL ZM1 ON ZM1.CODCOLIGADA = PF.CODCOLIGADA AND ZM1.CODIGO = '005'
    			LEFT JOIN ZMDCONFSIMULACAOPORTAL_EV ZM2 ON ZM2.CODCOLIGADA = ZM1.CODCOLIGADA AND ZM2.CODIGO = ZM1.CODIGO
            ...
            FROM 
                PFFINANC PF (NOLOCK)
                LEFT JOIN ZMDCONFSIMULACAOPORTAL ZM1 ON ZM1.CODCOLIGADA = PF.CODCOLIGADA AND ZM1.CODIGO = '006'
    			LEFT JOIN ZMDCONFSIMULACAOPORTAL_EV ZM2 ON ZM2.CODCOLIGADA = ZM1.CODCOLIGADA AND ZM2.CODIGO = ZM1.CODIGO
        ...
        FROM
            ...
            LEFT JOIN PSECAO D3 ON D3.CODIGO = D2.codsecao COLLATE Latin1_General_CI_AS AND D3.CODCOLIGADA = P.CODCOLIGADA
            LEFT JOIN PFUNCAO D4 ON D4.CODCOLIGADA = P.CODCOLIGADA AND D4.CODIGO = D.codfuncao COLLATE Latin1_General_CI_AS
        ...
        echo '<br><br><br><textarea>'.$query.'</textarea><br>';exit();
        $result = $this->dbrm->query($query);
        if(!$result) return false;
        return ($result->getNumRows() > 0) 
                ? $result->getResultArray() 
    ...
    public function ExecutaSimuladorCalculoSimulacao($acao, $filial, $secao, $funcao, $adicional = array()){
        ...
        $HE50 = (strlen(trim($adicional['he50_manual'] ?? '') > 0) ? str_replace(',', '.', str_replace('.', '', $adicional['he50_manual'])) : "
            ISNULL((
                ...
                WHERE
                        CHAPA IN (SELECT CHAPA FROM PFUNC WHERE 1 = (CASE WHEN P.CHAPA = CHAPA THEN 1 WHEN CODSECAO = '{$secao}' COLLATE Latin1_General_CI_AS THEN 1 ELSE 0 END) AND CODCOLIGADA = P.CODCOLIGADA AND CODSITUACAO <> 'D')
                    AND CONCAT(PF.ANOCOMP, REPLICATE('0', 2 - LEN(PF.MESCOMP)) + RTRIM(PF.MESCOMP)) >= SUBSTRING(CONVERT(VARCHAR, DATEADD(MONTH, -ZM1.QTDMESES, GETDATE()), 112),1,6)
                    AND PF.CODEVENTO = ZM2.CODEVENTO
                    AND PF.NROPERIODO IN (4,5,6,2)
                    --AND PF.REF = CASE WHEN ZM1.TIPO = '03' THEN 30 ELSE PF.REF END
