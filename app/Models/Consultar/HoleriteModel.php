...
class HoleriteModel extends Model {
    ...
    // -------------------------------------------------------
    public function ListarHoleriteConfiguracaoPeriodo($chapa = false){

        $filtro_periodo = "";
        if($chapa){

            $in_periodo = "";
            $PeriodosChapa = $this->ListarHoleritePeriodoChapa($chapa);
            if($PeriodosChapa){
                foreach($PeriodosChapa as $key => $DadosPeriodo){
                    $in_periodo .= "'{$DadosPeriodo['PERIODO']}',";
                }
            }

            if($in_periodo == "") return false;
            $filtro_periodo = " AND concat(anocomp, '-', mescomp, '-', periodo) IN (".rtrim($in_periodo, ',').") ";
        }

        $query = " SELECT * FROM zcrmportal_holerite_config_per WHERE coligada = '".session()->get('func_coligada')."' {$filtro_periodo} ORDER BY anocomp desc, mescomp desc, periodo asc ";
        $result = $this->dbportal->query($query);
        return ($result->getNumRows() > 0) 
                ? $result->getResultArray() 
                : false;
    ...
    public function ListarHolerite($dados){
        ...
        $query = "
            SELECT 
                ...
                PFPERFF.BASEFGTS ,
                --PFPERFF.BASEIRRF,
                (SELECT SUM(A.BASEIRRF) FROM PFPERFF A WHERE A.CHAPA = PFPERFF.CHAPA AND A.MESCAIXACOMUM = PFPERFF.MESCOMP AND A.ANOCAIXACOMUM = PFPERFF.ANOCOMP) AS BASEIRRF, 
                PFPERFF.BASEFGTS13 ,
                PFPERFF.BASEIRRF13 ,
                PFPERFF.BASEINSS, 
                PFPERFF.BASEINSS13, 
            ...
            SELECT 
                ...
                (SELECT SUM(A.BASEIRRF) FROM PFPERFFCOMPL A WHERE A.CHAPA = PFPERFFCOMPL.CHAPA AND A.MESCAIXACOMUM = PFPERFFCOMPL.MESCOMP AND A.ANOCAIXACOMUM = PFPERFFCOMPL.ANOCOMP) AS BASEIRRF, 
    ...
    // -------------------------------------------------------
    // Lista descritivos do holerite
    // -------------------------------------------------------
    public function ListarHoleriteDescritivo($dados){
        ...
        $query = "
            SELECT 
                PFFINANC.CODEVENTO, PEVENTO.DESCRICAO, PFFINANC.REF, PFFINANC.VALOR, PEVENTO.PROVDESCBASE
            FROM 
                PFFINANC, PEVENTO
            WHERE 
                ...
                AND PFFINANC.CHAPA='" . $chapa . "' AND PFFINANC.CODCOLIGADA = " . $coligada . "
                AND (PEVENTO.PROVDESCBASE = 'P' OR PEVENTO.PROVDESCBASE = 'D')
                AND PFFINANC.VALOR > 0	
            ORDER BY  PEVENTO.PROVDESCBASE DESC
        ";
        $result = $this->dbrm->query($query);
    ...
    // -------------------------------------------------------
    // Lista dados da empresa do holerite
    // -------------------------------------------------------
    public function ListarHoleriteDadosEmpresa(){
    ...
    // -------------------------------------------------------
    // Listar Períodos que o funcionários possui holerite
    // -------------------------------------------------------
    public function ListarHoleritePeriodoChapa($chapa){

        $query = " SELECT DISTINCT CONCAT(ANOCOMP, '-', MESCOMP, '-', NROPERIODO) PERIODO FROM PFPERFF WHERE CODCOLIGADA = '".session()->get('func_coligada')."' AND CHAPA = '".session()->get('func_chapa')."' ";
        $result = $this->dbrm->query($query);
