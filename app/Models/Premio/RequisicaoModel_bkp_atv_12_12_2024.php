...
class RequisicaoModel extends Model {
    ...
    public function ListarGerenteCoordenadores($id_req){
        ...
        $query = "
            ...
            LEFT JOIN CorporeRMQA2..PFUNC f ON f.codcoligada = r.id_coligada AND r.chapa_requisitor = f.chapa COLLATE Latin1_General_CI_AS 
            ...
            LEFT JOIN CorporeRMQA2..PFUNC a ON a.codcoligada = r.id_coligada AND g.ger_chapa = a.chapa COLLATE Latin1_General_CI_AS 
            LEFT JOIN CorporeRMQA2..PFUNC b ON b.codcoligada = r.id_coligada AND c.chapa = b.chapa COLLATE Latin1_General_CI_AS 
            WHERE r.id = ".$id_req." 
	...
	public function ImportarRequisicao($dados){
		...
		try {
            ...
            for ($i = 1; $i < count($data); $i++) { 
                ...
                if($chapa!='' and is_numeric($target) and is_numeric($realizado) and $target > 0 and $target <= 999.99 and $realizado >= 0 and $realizado <=999.99) {
    ...
    public function ProcessarRequisicao($dados){
        ...
        if($sucesso) session()->set(array('notificacao' => array('success', '<i class="mdi mdi-check-all alert-icon"></i> Processamento finalizado.')));

        return ($sucesso) 
                ? responseJson('success', 'Processamento finalizado', $this->dbportal->insertID())
