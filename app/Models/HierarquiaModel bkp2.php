...
class HierarquiaModel extends Model {
    ...
    public function ListarHierarquiaSecaoPodeVer($chapas = false, $Codcoligada = false, $hierarquiaLider = false){
        ...
        //$isLider = $this->dbportal->query(" SELECT * FROM zcrmportal_hierarquia_lider_func WHERE chapa = '{$chapa}' AND coligada = '{$coligada}' AND inativo IS NULL AND '".date('Y-m-d')."' BETWEEN A.perini AND (CASE WHEN A.perfim IS NOT NULL THEN A.perfim ELSE '2090-12-31' END) ");
        ...
        if($isLider && $hierarquiaLider){
			...
			SELECT * FROM (
				...
				FROM
					...
					INNER JOIN zcrmportal_hierarquia_lider_func C ON A.id = C.id_lider
				WHERE
						A.chapa = '{$chapa}'
				...
				SELECT
					C.chapa
				FROM
					zcrmportal_hierarquia_lider_excecao X
					INNER JOIN zcrmportal_hierarquia_lider A ON A.id = X.id_lider AND X.coligada = A.coligada
					INNER JOIN zcrmportal_hierarquia B ON B.id = A.id_hierarquia AND B.coligada = A.coligada
					INNER JOIN zcrmportal_hierarquia_lider_func C ON A.id = C.id_lider
				WHERE
						X.chapa = '{$chapa}'
	...
	public function isLider($chapa = false, $coligada = false){
		...
		$query = " SELECT * FROM zcrmportal_hierarquia_lider WHERE chapa = '{$chapa}' AND coligada = '{$coligada}' AND inativo IS NULL AND '".date('Y-m-d')."' BETWEEN perini AND (CASE WHEN perfim IS NOT NULL THEN perfim ELSE '2090-12-31' END) ";
		$result = $this->dbportal->query($query);
		return ($result->getNumRows() > 0) 
                ? true
	...
	public function isLiderExcecao($chapa = false, $coligada = false){
		...
		$query = " SELECT * FROM zcrmportal_hierarquia_lider_excecao WHERE chapa = '{$chapa}' AND coligada = '{$coligada}' AND inativo IS NULL AND '".date('Y-m-d')."' BETWEEN perini AND (CASE WHEN perfim IS NOT NULL THEN perfim ELSE '2090-12-31' END) ";
		$result = $this->dbportal->query($query);
		return ($result->getNumRows() > 0)
                ? true
                : false;
