                    ...
                    <div class="row">
                        ...
                        <div class="col-6 text-right">
                            <div class="button-items">
                                <a href="<?= base_url('variaveis/aluguel') ?>" class="btn btn-primary btn-xxs mb-0"><i class="far fa-arrow-alt-circle-left"></i> voltar</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="tab-content">
                        ...
                        <div class="tab-pane active p-3" id="auxilio_aluguel" role="tabpanel">
                            ...
                            <div class="form-group row mb-2">
                                ...
                                <div class="col-sm-10">
                                    <input onchange="verificaValor(this)"  class="form-control form-control-sm " data-money  type="text" value="" name="valor" id="valor" required>
                            ...
                            <input class="form-control datepicker m_data" hidden type="text" value="" name="grupoCargo" id="grupoCargo" required>
...
function verificaValor(input) {
    ...
    if (limitePermitido !== null && valor < limiteminimo) {
        Swal.fire({
            icon: 'error', // Ícone de erro para indicar que algo está errado
            title: 'Valor Excedido!',
...
const salvaDados = () => {
    ...
    $.ajax({
        url: "<?= base_url('variaveis/aluguel/save'); ?>",
        type:'POST',
        data:formData,
...
$("#anexo").fileinput({
    ...
    browseLabel: "Selecionar Arquivos", // Texto personalizado do botão de anexar
...
const selecionaFuncionario = (chapa) => {
    ...
    $.ajax({
        ...
        success:function(result){
            ...
            if (Array.isArray(regra.aluguel) && regra.aluguel.length > 0) {
             
                var valor = regra.reembolso_aluguel_valor_demais_filiais;

                // Itera sobre o array regra.aluguel
                regra.aluguel.forEach(function(aluguel) {
                    $('#quantMes').empty();
                    
                    if (aluguel.nome == response[0].CODFILIAL) {
                        valor = aluguel.valor; // Pega o campo valor dentro de regra.aluguel
                       
                        return false;
                    }
                });
                $('#quantMes').append('<option value="1">1 Mês</option>'); 
                // Adiciona options ao select a partir do valor de aluguel.valor
                if(valor >= 2){
