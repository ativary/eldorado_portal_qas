                ...
                <div class="card-header mt-0">
                    ...
                    <div class="col-6 text-right">
                        <div class="button-items">
                            <a href="<?= base_url('variaveis/decimoterceiro/novo') ?>" class="btn btn-primary btn-xxs mb-0"><i class="fas fa-plus-circle"></i> Nova Requisição</a>
                        </div>
                    </div>
                    </div>
                </div>
                                        ...
                                        <div class="btn-group dropleft mb-2 mb-md-0">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    <button onclick="verAnexos('<?= $dados->id; ?>','<?= $dados->status; ?>' )" disabled class="dropdown-item  " > Documentos</button>
                                                    ...
                                                    <a href="<?= base_url('variaveis/decimoterceiro/editar/'.base64_encode($dados->id)); ?>" class="dropdown-item <?= ($dados->status != '1') ? 'disabled' : ''; ?>">Editar requisição</a>
...
<div class="modal-dialog" style="max-width: 90%; width: 90%;">
        <div class="modal-content modal-content-full">
            <div class="modal-header bg-dark">
                <h5 class="modal-title mt-0 text-white"><i class="mdi mdi-file-document-box-outline"></i> Antecipação 1 parcela do 13º Salário </h5>
                <button type="button" class="close text-white" data-dismiss="modal"><i class="mdi mdi-close-circle-outline"></i></button>
            </div>
            <div class="modal-body">
...
<script>
    ...
    const selecionaFuncionario = (codigo) => {
        ...
        $.ajax({
            url: "<?= base_url('variaveis/decimoterceiro/selectFunc'); ?>",
            type:'POST',
            data:dados,
            success:function(result){
    ...
    $.ajax({
        url: "<?= base_url('variaveis/decimoterceiro/Anexos'); ?>",
        type: "POST",
        data: {
            "id": id
        },
    ...
    const Filtro = () => {
		...
		openLoading();
		$("#form_filtro").attr('action', base_url + '/variaveis/decimoterceiro').attr('target', '_self');
    ...
    const excluirAnexo = (id ) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "<?= base_url('variaveis/decimoterceiro/deleteAnexo'); ?>",
                    type: 'POST',
    ...
    const excluirReq = (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                openLoading();
                $.ajax({
                    url: "<?= base_url('variaveis/decimoterceiro/delete'); ?>",
                    type: 'POST',
                    data: dados, // Enviar o ID para exclusão
                    success: function(result) {
                        console.log(result);
