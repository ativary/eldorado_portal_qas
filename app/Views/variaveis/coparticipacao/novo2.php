 ...
 const tableBody = $('#dependentesTable tbody');
    ...
    reader.onload = function (e) {
        ...
        $.each(rows, function (index, row) {
            ...
            const valor = parseFloat(row[1]).toFixed(2);
                        ...
                        $.each(results, function (index, result) {
                            if (result && result.NOME) {
                                const newRow = $('<tr></tr>').html(`
                                    <td>${result.CHAPA}</td>
                                    <td>${result.NOME}</td>
                                    <td><input type="text" value="${result.VALOR}" data-money class="form-control form-control-sm valorDependente" placeholder="Valor" onchange="verificarLimite(this)"></td>
...
const salvaDados = () => {
    ...
    $("#dependentesTable tbody tr").each(function() {
        ...
        let dependente = {
            ...
            "valor": $(this).find("td:eq(2) input").val(),
    ...
    if (total == "" || total == 0) { 
        exibeAlerta("error", "<b>A lista precisa de funcionários com valores validos </b> ."); 
        return false; 
    }
    if (hasEmptyValue) {
        exibeAlerta("error", "<b>Todos os campos de valor devem ser preenchidos.</b>");
    ...
    if($("#tipoEvento").val() == ""){ exibeAlerta("error", "<b>Prestadora</b> obrigatório."); return false; }
    if(fora_periodo == 1){ exibeAlerta("error", "Fora do Perodo de abertura ."); return false; }
    openLoading();
