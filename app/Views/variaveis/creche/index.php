                ...
                <div class="card-header mt-0">
                    ...
                    <div class="col-6 text-right">
                        <div class="button-items">
                            <a href="<?= base_url('variaveis/creche/novo') ?>" class="btn btn-primary btn-xxs mb-0"><i class="fas fa-plus-circle"></i> Nova Requisição</a>
                        </div>
                    </div>
                    </div>
                </div>
                    ...
                    <table width="100%" cellspacing="0" cellpadding="0" id="datatable" class="table table-sm table-striped table-bordered">
                        <thead>
                            <tr class="text-center">
                                <th><input type="checkbox" onchange="toggleButtonVisibility()" id="select-all"></th>
                                        ...
                                        <button onclick="historicoAprovacao('<?= $dados->id; ?>')" class="btn btn-soft-primary pl-1 pr-1"  aria-expanded="true"> <i class="mdi mdi-history"></i></button>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    <button onclick="aprovarReq('<?= $dados->id; ?>')" class="dropdown-item" style="color: blue;"> Enviar para aprovação</button>
                                                    ...
                                                    <a  href="<?= base_url('variaveis/creche/editar/'.base64_encode($dados->id)); ?>" class="dropdown-item <?= ($dados->status != '1') ? 'disabled' : ''; ?>">Editar requisição</a>
...
<div class="modal-dialog" style="max-width: 90%; width: 90%;">
        <div class="modal-content modal-content-full">
            <div class="modal-header bg-dark">
                <h5 class="modal-title mt-0 text-white"><i class="mdi mdi-file-document-box-outline"></i> Auxilio Creche</h5>
                <button type="button" class="close text-white" data-dismiss="modal"><i class="mdi mdi-close-circle-outline"></i></button>
            </div>
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <ul class="nav nav-tabs" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab" href="#auxilio_pcd" role="tab"><i class="fa fa-home"></i> Auxilio Creche</a>
                            </li>
...
<script>
    ...
    const abrirReq = (dados) => {
        ...
        const novoTitulo = `Auxilio Creche - ${req.id}`; // Exemplo de como construir o novo título
    ...
    const Filtro = () => {
		...
		openLoading();
		$("#form_filtro").attr('action', base_url + '/variaveis/creche').attr('target', '_self');
                            ...
                            $.ajax({
                                ...
                                success: function(result) {
                                    ...
                                    if (response.tipo != 'success') {
                                        ...
                                        exibeAlerta(response.tipo, response.msg, 3, '<?= base_url('variaveis/creche'); ?>');
                                    }
                                },
                                error: function(xhr, status, error) {
    ...
    const excluirReq = (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                openLoading();
                $.ajax({
                    url: "<?= base_url('variaveis/creche/delete'); ?>",
                    type: 'POST',
                    data: dados, // Enviar o ID para exclusão
                    success: function(result) {
                        console.log(result);
