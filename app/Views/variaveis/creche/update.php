            ...
            <div class="card">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6 text-right">
                            <div class="button-items">
                                <a href="<?=($req[0]->status == '3' || $req[0]->status == '7'  ) ? base_url('variaveis/sincronizacao') : base_url('variaveis/creche');  ?>" class="btn btn-primary btn-xxs mb-0"><i class="far fa-arrow-alt-circle-left"></i> voltar</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <ul class="nav nav-tabs" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link active" data-toggle="tab" href="#reembolso_cpd" role="tab"><i class="fa fa-child"></i> Auxilio Creche</a>
                        </li>
                            ...
                            <div class="form-group row mb-2">
                                ...
                                <div class="col-sm-8">
                                    <select  class="select2 custom-select form-control form-control-sm" name="dependente" id="dependente">
                                        <option value=""> ... </option>
                                        <?php if ($dependentes) : ?>
                                            <?php foreach ($dependentes as $key2 => $depend) : ?>
                                                <option value='<?= json_encode($depend) ?>' ><?= $depend['NRODEPEND'] ?> - <?= $depend['NOMEDEPEND'] ?></option>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    </select>
                                    ...
                                    <table class="table table-bordered" id="dependentesTable">
                                        ...
                                        foreach ($dependentes as $key2 => $dados2) : ?>
                                            <tr>
                                                ...
                                                <td class="n-mobile-cell"><?= $dados2->data_nascimento ?></td>
                                                <td class="n-mobile-cell"><?= $dados2->parentesco ?></td>
                                                <td class="n-mobile-cell" ><input onchange="verificarLimite(this)" type="text" data-money class=" form-control form-control-sm valorDependente" value="<?= $dados2->valor ?>" placeholder="Valor"></td>
                ...
                <div class="card-footer text-center">
                    <button  class="btn btn-success bteldorado_1" id="btnsave" onclick="return salvaDados()"><i class="fas fa-check"></i> Atualizar</button>
                </div>
...
<script>
    ...
    function verificaData() {
        ...
        const tipoReq = <?= $req[0]->tiporeq ?>; 
        const periodoInicio = parseInt(regra.periodo_creche, 10);
        const periodoFim = parseInt(regra.periodo_creche_fim, 10);
    
        const $selectTipoReq = $('#tipoReq');
        const $opcaoMensal = $selectTipoReq.find('option[value="1"]');

        // opção "Complementar"
        const dia_lim_compl = parseInt(regra.dia_limite_compl2, 10);
    ...
    const salvaDados = () => {
     ...
     let hasEmptyValue = false; // Flag para identificar valor vazio
        ...
        if (total == "" || total == 0) { 
            exibeAlerta("error", "<b>Todos os dependentes precisam de valores </b> ."); 
            return false; 
        }
        if ($("#funcionario").val() == "") { 
        ...
        $.ajax({
            url: "<?= base_url('variaveis/creche/update'); ?>",
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(result) {
                ...
                if (response.tipo != 'success') {
                    ...
                    exibeAlerta(response.tipo, response.msg, 3, '<?=($req[0]->status == '3' || $req[0]->status == '7'  ) ? base_url('variaveis/sincronizacao') : base_url('variaveis/creche');?>');
                }
            },
        });
    }
...
function verificarLimite(input) {
    ...
    $.each(regra.dependentes, function(index, func) {
        if (func.nome == filial) {
            ...
            limitePermitido = parseFloat(regra.reembolso_cpd_valor_demais_filiais.replace(/[^0-9,-]+/g,"").replace(",","."));
        }
    });
...
const selecionaFuncionario = (chapa) => {
    ...
    $.ajax({
        url: "<?= base_url('variaveis/creche/dadosFunc'); ?>",
        type:'POST',
        data:dados,
        success:function(result){
           console.log(result)
