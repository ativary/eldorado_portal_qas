                ...
                <div class="card-header mt-0">
                    ...
                    <div class="col-6 text-right">
                        <div class="button-items">
                            <a href="<?= base_url('variaveis/substituicao/novo') ?>" class="btn btn-primary btn-xxs mb-0"><i class="fas fa-plus-circle"></i> Nova Requisição</a>
                        </div>
                    </div>
                    </div>
                </div>
                                        ...
                                        <div class="btn-group dropleft mb-2 mb-md-0">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    <?php endif; ?>
                                                    <a href="<?= base_url('variaveis/substituicao/editar/'.base64_encode($dados->id)); ?>" class="dropdown-item <?= ($dados->status != '1') ? 'disabled' : ''; ?>">Editar requisição</a>
        ...
        <div class="modal-content modal-content-full">
            ...
            <div class="modal-body">
                ...
                <div class="card-footer text-center">
                    ...
                    <button type="button" class="btn btn-primary" style="visibility: hidden;" id="btnTermoSubstituicao"><i class="fas fa-file-pdf"></i> Termo de Substituição</button>
                </div>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
...
<div class="modal-dialog" style="max-width: 90%; width: 90%;">
        <div class="modal-content modal-content-full">
            <div class="modal-header bg-dark">
                <h5 class="modal-title mt-0 text-white"><i class="mdi mdi-file-document-box-outline"></i> Salário  Substituição</h5>
                <button type="button" class="close text-white" data-dismiss="modal"><i class="mdi mdi-close-circle-outline"></i></button>
            </div>
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="tab-content">
                            ...
                            <div class="tab-pane active p-3" id="auxilio_substituicao" role="tabpanel">
                                ...
                                <div class="form-group row mb-2">
                                    <label for="Reqfuncionario" class="col-sm-2 col-form-label text-right text-left-sm">Funcionário Substituido:</label>
                                    <div class="col-sm-10">
                                        <select disabled class="select2 custom-select form-control form-control-sm" name="ReqfuncionarioSub" id="ReqfuncionarioSub">               
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row mb-2">
                                    <label for="Reqfuncionario" class="col-sm-2 col-form-label text-right text-left-sm">Funcionário substituto :</label>
                                    <div class="col-sm-10">
                                ...
                                <div class="form-group row mb-2">
                                    <label for="data" class="col-sm-2 col-form-label text-right text-left-sm"> Período:</label>
                                    <div class="input-group col-sm-10 ">
                                        <input disabled class="form-control datepicker m_data" type="date"  value="" name="req_data_inicio" id="req_data_inicio" required>
                                        <div class="input-group-prepend input-group-append">
                                            <span class="input-group-text">até</span>
                                        </div>
                                        <input disabled class="form-control datepicker m_data" type="date"  value="" name="req_data_fim" id="req_data_fim" require>
                                    </div>
                                </div>
...
<script>
    ...
    $.ajax({
        url: '<?= base_url('variaveis/substituicao/substituicaoTermo') ?>',
        type: 'GET',
        ...
        success: function(response) {
            ...
            link.href = url;
            link.download = 'termo_de_substituicao.pdf';
...
$.ajax({
    ...
    const salvaAnexo = () => {
        ...
        $.ajax({
            url: "<?= base_url('variaveis/substituicao/salvarAnexo'); ?>",  // Altere para a URL do seu endpoint de upload
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $.ajax({
        url: "<?= base_url('variaveis/substituicao/Anexos'); ?>",
        type: "POST",
        data: {
            "id": id
        },
    ...
    const abrirReq = (dados) => {
        ...
        const novoTitulo = `Salário  Substituição - ${req.id}`; // Exemplo de como construir o novo título
        ...
        $("#ReqfuncionarioSub").append($('<option></option>')
        .attr('value', valor.funcionario_sub)
        .attr('selected', true) // Define a opção como selecionada
        .text(valor.Nome_sub));
        $("#req_data_inicio").val(valor.data_inicio);
        $("#req_data_fim").val(valor.data_fim);
        if(valor.tipoReq == '1'){
            tipo = 'Mensal';
        }else{
            tipo = 'Complementar';
    ...
    const Filtro = () => {
		...
		openLoading();
		$("#form_filtro").attr('action', base_url + '/variaveis/substituicao').attr('target', '_self');
    ...
    const excluirAnexo = (id ) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "<?= base_url('variaveis/substituicao/deleteAnexo'); ?>",
                    type: 'POST',
    ...
    const excluirReq = (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                openLoading();
                $.ajax({
                    url: "<?= base_url('variaveis/substituicao/delete'); ?>",
                    type: 'POST',
                    data: dados, // Enviar o ID para exclusão
                    success: function(result) {
                        console.log(result);
