                ...
                <div class="card-header mt-0">
                    ...
                    <div class="col-6 text-right">
                        <div class="button-items">
                            <a href="<?= base_url('manager/perfil/novo') ?>" class="btn btn-primary btn-xxs mb-0"><i class="fas fa-plus-circle"></i> Novo</a>
                        </div>
                    </div>
                    </div>
                </div>
                            ...
                            <?php if($resDados): ?>
                                <?php foreach($resDados as $key => $Dados): ?>
                                    <tr data-linha="<?= $Dados['id'] ?>">
                                        ...
                                        <td><a href="<?= base_url('manager/perfil/editar/'.$Dados['id']); ?>"><?= $Dados['nome'] ?></a></td>
                                        <td><?= $Dados['descr'] ?></td>
                                        <td class="text-center">
                                            <div class="btn-group" aria-label="acao" role="group">
                                                ...
                                                <a href="<?= base_url('manager/perfil/editar/'.$Dados['id']); ?>" class="btn btn-soft-primary waves-effect waves-light btn-xxs"><i class="mdi mdi-pencil-outline"></i> editar</a>
                                                <a href="<?= base_url('manager/perfil/funcoes/'.$Dados['id']); ?>" class="btn btn-soft-warning waves-effect waves-light btn-xxs"><i class="mdi mdi-source-branch"></i> funções</a>
                                            </div>
...
const excluir = (id) => {
    ...
    Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: "<?= base_url('manager/perfil/action/deletar') ?>",
				type:'POST',
				data:dados,
