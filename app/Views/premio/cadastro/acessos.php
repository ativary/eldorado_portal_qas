                    ...
                    <div class="row">
                        ...
                        <div class="col-6 text-right">
                            <div class="button-items">
                               <a href="<?= base_url('premio/cadastro/novo_acesso/'.$id_premio) ?>" class="btn btn-primary btn-xxs mb-0"><i class="fas fa-plus-circle"></i> Novo Acesso</a>
                            ...
                            <?php if($resAcessos): ?>
                                <?php foreach($resAcessos as $key => $Acesso): ?>
                                    <tr data-linha="<?= $Acesso['id'] ?>">
                                        <td class="text-left"><a href="<?= base_url('premio/cadastro/editar_acesso/'.$Acesso['id'].'/'.$id_premio); ?>"><?= $Acesso['id'] ?></a></td>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group" aria-label="acao" role="group">
                                                <a href="<?= base_url('premio/cadastro/acessos_usuarios/'.$Acesso['id'].'/'.$id_premio); ?>" class="btn btn-soft-success waves-effect waves-light btn-xxs"><i class="fa fa-users"></i> Usuários</a>
                                                ...
                                                <a href="<?= base_url('premio/cadastro/editar_acesso/'.$Acesso['id'].'/'.$id_premio); ?>" class="btn btn-soft-primary waves-effect waves-light btn-xxs"><i class="mdi mdi-pencil-outline"></i> Editar</a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
...
const excluir = (id) => {
    ...
    Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: "<?= base_url('premio/cadastro/action_acessos/deletar') ?>",
				type:'POST',
				data:dados,
