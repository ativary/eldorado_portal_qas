...
const salvaDados = () => {
    ...
    <?php if ($resAcessos) {?>
        <?php foreach($resAcessos as $key => $Acesso): ?>
            ...
            <?php if ($Acesso['id'] != $id) {?>
                ...
                if($("#dtper_ponto").val().substring(0, 10) == "<?= $Acesso['dtini_ponto']; ?>"){ exibeAlerta("error", "Esse <b>Período de Ponto</b> já existe em outro período de acessos de exceção cadastrado."); return false; }
            <?php }?>
        <?php endforeach; ?>
    <?php }?>
    ...
    $.ajax({
        url: "<?= base_url('premio/cadastro/action_acessos_excecao/editar'); ?>",
        type:'POST',
        data:dados,
