                    ...
                    <div class="row">
                        ...
                        <div class="col-6 text-right">
                            <div class="button-items">
                                ...
                                <?php if($pode_editar) {?>
                                    <a href="javascript:void(0);" onclick="return processar(<?= $id_requisicao; ?>,'<?= $resRequisicao[0]['tipo_requisicao']; ?>')" class="btn btn-purple btn-xxs mb-0 <?= $em_analise; ?>"><i class="fas fa-table"></i> Processar</a>
                                <?php }?>
...
const processar = (id_requisicao, tipo_req) => {

    console.log( tipo_req); 
    tipo_req = (tipo_req == 'MENSAL') ? 'M' : 'C';
    console.log( tipo_req);    
...
const apagar_zerados = (id_requisicao) => {

Swal.fire({
    icon: 'question',
    title: 'Esta ação apaga todas as chapas com percentual realizado igual a zero. Deseja realmente apagar esse(s) <b>registro(s)</b>?',
    showDenyButton: true,
    showCancelButton: true,
    confirmButtonText: `Sim`,
    denyButtonText: `Cancelar`,
    ...
    if (result.isConfirmed) {
        ...
        $.ajax({
            url: "<?= base_url('premio/requisicao/action/apagar_zerados') ?>",
            type:'POST',
            data:dados,
            success:function(result){
                var response = JSON.parse(result);
