...
const incluirNovaBatida = () => {
	...
	var html = '<tr data-p="I" data-info="' + id + '">' +
		'<td><input type="date" class="form-control" value="' + data_nova_batida + '"></td>' +
		'<td><input type="time" class="form-control" value=""></td>' +
		'<td><select class="form-control"><option value="0" ' + (natureza_proxima_batida != 1 ? 'selected' : '') + '>Entrada</option><option value="1" ' + (natureza_proxima_batida == 1 ? 'selected' : '') + '>Saida</option></select></td>' +
		'<td width="18" class="p-0"><button class="btn btn-danger btn-xxs pr-1 pl-1" data-remove-batida="' + id + '">X</button></td>' +
		'</tr>' +
		'<tr data-justificativa="' + id + '" data-just><td colspan="3" style="border-top: none;" class="pb-3"><label><i class="mdi mdi-arrow-up-bold"></i> Motivo do ajuste:</label><select class="form-control mt-"><option value="">...</option>'+select_justificativa_ajuste+'</td></tr>';
...
const validaDados = () => {
	...
	$("[data-p]").each(function(e) {
		...
		if (id != undefined) {
			...
			$("[data-info='" + id + "']").find('input,select')[0].setAttribute('class', 'form-control');
			$("[data-info='" + id + "']").find('input,select')[1].setAttribute('class', 'form-control');
			$("[data-info='" + id + "']").find('input,select')[2].setAttribute('class', 'form-control');

			if (data_ref == "") $("[data-info='" + id + "']").find('input,select')[0].setAttribute('class', 'form-control parsley-error text-danger');
			if (batida == "") $("[data-info='" + id + "']").find('input,select')[1].setAttribute('class', 'form-control parsley-error text-danger');
			if (natureza == "") $("[data-info='" + id + "']").find('input,select')[2].setAttribute('class', 'form-control parsley-error text-danger');
...
const abrirAlteracaoBatida = (data, data_br, diasemana, batidas, chapa, escala, inicioEscala, terminoEscala, cpf, is_motorista) => {
	...
	for (var x = 0; x < batidas.length; x++) {
		...
		var html = '<tr data-info="' + id_tr + '" ' + tipo + '>' +
			'<td><input '+((batidas[x].motivo_reprova != '' && batidas[x].motivo_reprova != null) ? ' disabled ' : '')+' type="date" class="form-control" value="' + batidas[x].data_ponto + '"></td>' +
			...
			'<td><select '+((batidas[x].motivo_reprova != '' && batidas[x].motivo_reprova != null) ? ' disabled ' : '')+' class="form-control" style="min-width:56px;"><option value="0" ' + (batidas[x].natureza != 1 ? 'selected' : '') + '>Entrada</option><option value="1" ' + (batidas[x].natureza == 1 ? 'selected' : '') + '>Saida</option></select></td>' +
...
const alterarBatida = () => {
	...
	$("[data-p]").each(function(e) {
		...
		if (id != undefined) {
			...
			if (data_ref == "") {
				$("[data-info='" + id + "']").find('input,select')[0].setAttribute('class', 'form-control parsley-error text-danger');
			} else {
				$("[data-info='" + id + "']").find('input,select')[0].setAttribute('class', 'form-control');
			}

			if (batida == "") {
				$("[data-info='" + id + "']").find('input,select')[1].setAttribute('class', 'form-control parsley-error text-danger');
			} else {
				$("[data-info='" + id + "']").find('input,select')[1].setAttribute('class', 'form-control');
			}

			if (natureza == "") {
				$("[data-info='" + id + "']").find('input,select')[2].setAttribute('class', 'form-control parsley-error text-danger');
			} else {
				$("[data-info='" + id + "']").find('input,select')[2].setAttribute('class', 'form-control');
			}
