                                ...
                                <tbody>
                                    <?php foreach ($resDiasEspelho as $key => $DiasEspelho) : ?>
                                        ...
                                        <tr>
                                            ...
                                            <td <?= $bglinha; ?> class="n-mobile-cell text-center text-info"><?= ((int)$DiasEspelho['total_direcao'] > 0) ? ($DiasEspelho['total_direcao'].':00') : ""; ?></td>
                                            <td <?= $bglinha; ?> class="n-mobile-cell text-center text-info"><?= ((int)$DiasEspelho['total_espera'] > 0) ? ($DiasEspelho['total_espera'].':00') : ""; ?></td>
                                                    ...
                                                    <?php if(!$periodo_bloqueado || $rh || 1==1): ?>
                                                        <div class="btn-group dropleft mb-2 mb-md-0">
                                                            ...
                                                            <?php else: ?>
                                                                <button type="button" class="btn btn-soft-primary dropdown-toggle pl-1 pr-1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><i class="mdi mdi-dots-vertical"></i></button>
                                                                    <div class="dropdown-menu">
                                                                        <button onclick="abrirResumoDiario('<?= dtBr($DiasEspelho['DATA']); ?>', '<?= diaSemana($DiasEspelho['DATA']); ?>', '<?= ((int)$DiasEspelho['HTRAB'] > 0) ? m2h($DiasEspelho['HTRAB']) : '0:00'; ?>', '<?= ((int)$DiasEspelho['ATRASO'] > 0) ? m2h($DiasEspelho['ATRASO']) : '0:00'; ?>', '<?= ((int)$DiasEspelho['FALTA'] > 0) ? m2h($DiasEspelho['FALTA']) : '0:00'; ?>', '<?= ((int)$DiasEspelho['EXTRAAUTORIZADO'] > 0) ? m2h($DiasEspelho['EXTRAAUTORIZADO']) : '0:00'; ?>', '<?= ((int)$DiasEspelho['ABONO'] > 0) ? m2h($DiasEspelho['ABONO']) : '0:00'; ?>', '<?= urlencode(json_encode($array_batidas)); ?>', '<?= $DiasEspelho['ESCALA']; ?>', '<?= ((int)$DiasEspelho['total_refeicao'] > 0) ? m2h($DiasEspelho['total_refeicao']) : "0:00"; ?>', '<?= ((int)$DiasEspelho['total_direcao'] > 0) ? m2h($DiasEspelho['total_direcao']) : "0:00"; ?>', '<?= ((int)$DiasEspelho['total_espera'] > 0) ? m2h($DiasEspelho['total_espera']) : "0:00"; ?>', '<?= m2h($DiasEspelho['ADICIONAL']); ?>', '<?= m2h($DiasEspelho['SUSPENSAO']) ?>')" type="button" class="dropdown-item" title="Resumo diário"><i class="mdi mdi-file-document-box-outline"></i> Resumo diário</button>
                                                                    </div>
                                                            <?php endif;?>
                                                    <?php else: ?>
                                                        <i title="Periodo bloqueado" class="mdi mdi-lock-alert" disabled style="font-size:20px;color:#999999;"></i>
...
<script>
    ...
    <?php if ($resDiasEspelho && $chapa) : ?>
        ...
        // resumo diario
        //-----------------------------------------------------------
        const abrirResumoDiario = (data, diasemana, horas_trabalhadas, atrasos, faltas, extras, abono, batidas, escala, total_refeicao, total_direcao, total_espera, adicional_noturno, suspensao) => {
