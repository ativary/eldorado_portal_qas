            ...
            <table id="datatableAprovacao" class="table table-sm table-bordered table-responsive_mobile" style="width: 100%;">
              <thead>
                <tr>
                  ...
                  <th class="n-mobile-cell"><strong>Horas Extras</strong></th>
                  <th class="n-mobile-cell"><strong>Horas Digitadas</strong></th>
                  <th>Ação</th>
                </tr>
              </thead>
              <tbody>
                <?php if ($resReqChapas): ?>
                  <?php foreach ($resReqChapas as $key => $registro): ?>
                    <tr data-linha="<?= $registro['id'] ?>">
                      ...
                      <td class="n-mobile-cell"><?= $registro['horas_extras_do_dia']; ?></td>                
                      <td class="n-mobile-cell">
                          <button class="btn btn-soft-primary" 
                                  type="button"
                                  onclick="atuHorasDigitadas(
                                    '<?= $registro['id']; ?>', 
                                    '<?= $registro['chapa_colab'] . ' - ' . $registro['nome_colab']; ?>', 
                                    '<?= $registro['dt_ponto_br']; ?>', '<?= $registro['horas_extras_do_dia']; ?>', 
                                    '<?= $registro['horas_digitadas']; ?>'
                                  )" >
                                  <?= $registro['horas_digitadas'] == '' ? '__ : __' : $registro['horas_digitadas'] ; ?>
                          </button>
                      </td>
                      <td>
                        <div class="dropdown">
...
<!-- modal -->
<div class="modal" id="modalHoras" style="width:100%;" role="dialog" aria-labelledby="modalHoras" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title text-center" id="modalHoraslabel"><span class="oi oi-people"></span> Horas Digitadas </h5>
      ...
      <div class="modal-body">
        <div class="input-group mb-0">
          <label name="h_colab" id="h_colab">Colaborador: </label>
        </div>
        <div class="input-group mb-0">
          <label name="h_data" id="h_data">Data: </label>
        </div>
        <div class="input-group mb-0">
          <label name="h_he_dia" id="h_he_dia">Horas Extras do dia: </label>
        </div>
        <div class="input-group mb-3">
          <div class="input-group-prepend">
            <label class="input-group-text" style="width: 150px;" for="h_h_dig">Horas Digitadas: </label>
          </div>
          <input class="form-control" type="time"  value="" name="h_h_dig" id="h_h_dig" required>
          <input type="hidden" value="" name="h_h_dia" id="h_h_dia">
      ...
      <div class="modal-footer">
        ...
        <button type="button" name="btnConfirmar" id="btnConfirmar" class="btn btn-success" onclick="return GravaHoraDigitada();"> <i class="fa fa-check"></i> Confirmar </button>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  const atuHorasDigitadas = (h_id, h_colab, h_data, h_he_dia, h_h_dig) => {
    //abre modal
    console.log(h_id, h_colab, h_data, h_he_dia, h_h_dig);
    $("#h_colab").html(`<b>Colaborador:</b> ${h_colab}`);
    $("#h_data").html(`<b>Data:</b> ${h_data}`);
    $("#h_he_dia").html(`<b>Horas Extras do dia:</b> ${h_he_dia}`);
    $("#h_h_dia").val(h_he_dia);
    $("#h_h_dig").val(h_h_dig);
    $("#h_id").val(h_id);
    $("#modalHoras").modal();
  }
  ...
  $('#modalHoras').on('shown.bs.modal', function () {
    $('#h_h_dig').trigger('focus');  // your input
  });

  // Catch Tab key on the input
  $('#h_h_dig').on('keydown', function (e) {
    if (e.key === "Tab") {
      e.preventDefault(); // stop normal tabbing
      $('#btnConfirmar').focus();   // your button ID/class
    }
  });
  ...
  const GravaHoraDigitada = () => {
    let hora_dia = $("#h_h_dia").val();
    let hora_digi = $("#h_h_dig").val();
    let id = $("#h_id").val();

    if (!validaHora(hora_digi)) {
      exibeAlerta("error", "Hora inválida.");
      return false;
    }

    if (hora_digi > hora_dia) {
      exibeAlerta("error", "Hora não pode ser maior que horas extras do dia.");
      return false;
    }
    ...
    $.ajax({
      url: "<?= base_url('ponto/art61/action/grava_hora_req_chapa'); ?>",
      type: 'POST',
      data: dados,
      success: function(result) {
        var response = JSON.parse(result);
        if (response.tipo != 'success') {
          exibeAlerta("error", "Não foi possível atualizar horas digitadas.");
          return false;
        } else {
          $(`tr[data-linha="${id}"] td:nth-child(12) button`).text(hora_digi);
