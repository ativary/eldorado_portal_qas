          ...
          <div class="row">
            ...
            <div class="col-6 text-right">
              <div class="button-items">
                <a href="<?= base_url('ponto/art61/solicitacao_chapas') . '/' . $id_req ?>" class="btn btn-primary btn-xxs mb-0"><i class="far fa-arrow-alt-circle-left"></i> voltar</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  const importarJust = () => {
    ...
    Swal.fire({
      icon: 'question',
      title: 'Confirma a importação de Justificativas?',
      showDenyButton: true,
      ...
      if (result.isConfirmed) {
        ...
        fd.append('id_requisicao', '<?= $id_req ?>');
        fd.append('arquivo_importacao', img[0].files[0]);

        $.ajax({
          url: "<?= base_url('ponto/art61/action/importar_justificativas') ?>",
          type: 'POST',
          processData: false,
          contentType: false,
          data: fd,
          success: function(result) {
            ...
            exibeAlerta(response.tipo, response.msg, 6, '<?= base_url('ponto/art61/solicitacao_chapas') . '/' . $id_req ?>');
